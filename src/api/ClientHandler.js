...
class ClientHandler extends EventEmitter {
    ...
    async connect(authflow, connection, deviceProfile = {}) {
        if (this.client) {
            throw new Error('ClientHandler is already connected')
        }

        const client = await createRealmClient(
            authflow,
            connection,
            deviceProfile
        )
    ...
    attach(client) {
        if (!client || typeof client.on !== 'function') {
            throw new TypeError('A valid Realm client is required')
        }
    ...
    _attach(client) {
        this._on('realm_chat', data => {
            this.emit('chat', new ChatMessage(data))
        })
        ...
        this._on('realm_death', data => {
            ...
            this.emit(
                ...
                new DeathEvent({
                    ...
                    message:
                        data?.message ??
                        formatDeathMessage(key, params),
                    raw: data
                })
            )
        })

        this._on('realm_system', data => {
            this.emit('system', {
                key: data?.key ?? null,
        ...
        this._on('realm_join', data => {
            const player = this.players.upsert({
                ...
                gamertag:
                    data?.player ??
                    data?.profile?.gamertag,
                xuid: data?.profile?.xuid,
                platformChatId:
                    data?.profile?.platformChatId,
                buildPlatform:
                    data?.profile?.buildPlatform,
                skinData:
                    data?.profile?.skinData,
                initial: false,
                online: true
            })
        ...
        this._on('realm_leave', data => {
            const existing = this.players.get(
                data?.profile?.uuid ??
                data?.player
            )
        ...
        this._on('start_game', data => {
            ...
            // Preserve the low-level packet for API consumers that need
            // protocol data such as the Realm world seed.
    ...
    sendChat(message, senderName = this.options.senderName) {
        ...
        return this.client.sendRelayMessage(
            text,
            senderName || this.client.username
        )
    }
    ...
    externalCommand(command, options = {}) {
        ...
        if (!value) {
            throw new Error('External command cannot be empty')
        }

        return this.client.sendExternalCommand(
            value,
            options
        )
    }
    ...
    externalMe(message, options = {}) {
        ...
        if (!text) {
            throw new Error(
                'External /me message cannot be empty'
            )
        }
    ...
    externalTell(player, message, options = {}) {
        ...
        if (!target) {
            throw new Error(
                'External /tell target cannot be empty'
            )
        }

        if (!text) {
            throw new Error(
                'External /tell message cannot be empty'
            )
        }
